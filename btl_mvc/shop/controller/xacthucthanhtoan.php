...
if($check){
    ...
    $User=$order_code;
    $sql_dathang='Call CapNhatTrangThaiGioHang("'.$User.'",@NgayThanhToan)';
    //echo $sql_dathang;
    $kq_dathang=mysql_query($sql_dathang);

    //echo "<script>window.location='hoadon.php?user=$NewUser';</script>";
    if($kq_dathang){
        $row=mysql_fetch_array(mysql_query("SELECT @NgayThanhToan as KQ"));
        $NgayThanhToan=$row['KQ'];
        header("Location:../view/HoaDon.php?NgayThanhToan=".$NgayThanhToan);
    }else{
        echo "Quá trình thanh toán không thành công bạn vui lòng thực hiện lại";
    }
}
else{
    echo "Quá trình thanh toán không thành công bạn vui lòng thực hiện lại";
